...
public class CarouselDAO extends BaseHibernateDAO {
	...
	public Map<String, Object> findInfoByConditions(Carousel selectinfo, int start, int limit) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		List<Carousel> infoList = new ArrayList<Carousel>();
		int infoListCount = 0;
		try {
			Criteria listCriteria = getSession().createCriteria(Carousel.class);
			Criteria countCriteria = getSession().createCriteria(Carousel.class);
...
//	            String value = (String) m.invoke(selectinfo); 
			...
			infoList = (List<Carousel>) criteriaList(listCriteria, start, limit);
			
			map.put("carouselList", infoList);
	...
	public void delete(String delData){
		...
		try {
			for( int i=0; i<id.length; i++){
				Carousel newinfo = findById(Integer.parseInt(id[i]));
				newinfo.setDr(1);
	...
	public Carousel findById(java.lang.Integer id) {
		try {
			Carousel instance = (Carousel) getSession().get("com.yinli.item.vo.Carousel", id);
			return instance;
		} catch (RuntimeException re) {
			throw re;
		}
