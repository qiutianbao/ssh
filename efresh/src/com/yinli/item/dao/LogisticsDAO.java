...
@Repository
public class LogisticsDAO extends BaseHibernateDAO {
	
	public Map<String, Object> findInfoByConditions(Logistics selectinfo, int start, int limit) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		List<Logistics> infoList = new ArrayList<Logistics>();
		int infoListCount = 0;
		try {
			Criteria listCriteria = getSession().createCriteria(Logistics.class);
			Criteria countCriteria = getSession().createCriteria(Logistics.class);
			...
			infoList = (List<Logistics>) criteriaList(listCriteria, start, limit);
			
			map.put("logisticsList", infoList);
	...
	public void delete(String delData){
		...
		try {
			for( int i=0; i<id.length; i++){
				Logistics newinfo = findById(Integer.parseInt(id[i]));
				newinfo.setDr(1);
	...
	public Logistics findById(java.lang.Integer id) {
		try {
			Logistics instance = (Logistics) getSession().get("com.yinli.item.vo.Logistics", id);
			return instance;
		} catch (RuntimeException re) {
			throw re;
		}
