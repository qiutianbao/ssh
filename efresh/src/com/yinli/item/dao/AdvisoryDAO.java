...
@Repository
public class AdvisoryDAO extends BaseHibernateDAO {
	
	public Map<String, Object> findInfoByConditions(Advisory selectinfo, int start, int limit) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		List<Advisory> infoList = new ArrayList<Advisory>();
		int infoListCount = 0;
		try {
			Criteria listCriteria = getSession().createCriteria(Advisory.class);
			Criteria countCriteria = getSession().createCriteria(Advisory.class);
			...
			infoList = (List<Advisory>) criteriaList(listCriteria, start, limit);
			
			map.put("advisoryList", infoList);
	...
	public void save(Advisory transientInstance) {
		try {
	...
	public void delete(String delData){
		...
		try {
			for( int i=0; i<id.length; i++){
				Advisory newinfo = findById(Integer.parseInt(id[i]));
				newinfo.setDr(1);
	...
	public Advisory findById(java.lang.Integer id) {
		try {
			Advisory instance = (Advisory) getSession().get("com.yinli.item.vo.Advisory", id);
			return instance;
		} catch (RuntimeException re) {
			throw re;
		}
