...
@Repository
public class UserscoreDAO extends BaseHibernateDAO {
	
	public Map<String, Object> findInfoByConditions(Userscore selectinfo, int start, int limit) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		List<Userscore> infoList = new ArrayList<Userscore>();
		int infoListCount = 0;
		try {
			Criteria listCriteria = getSession().createCriteria(Userscore.class);
			Criteria countCriteria = getSession().createCriteria(Userscore.class);
			...
			infoList = (List<Userscore>) criteriaList(listCriteria, start, limit);
			
			map.put("userscoreList", infoList);
	...
	public void delete(String delData){
		...
		try {
			for( int i=0; i<id.length; i++){
				Userscore newinfo = findById(Integer.parseInt(id[i]));
				newinfo.setDr(1);
	...
	public Userscore findById(java.lang.Integer id) {
		try {
			Userscore instance = (Userscore) getSession().get("com.yinli.item.vo.Userscore", id);
			return instance;
		} catch (RuntimeException re) {
			throw re;
		}
