...
var querymarketingStore;			
var querymarketingGrid;
var addmarketing;
var addmarketingForm;
var addmarketingWin;
var updatemarketing;
var updatemarketingWin;
var updatemarketingForm;
var updatemarketingReader;
var showQueryPanel;
...
var selectUnitForm;
 ...
 Ext.onReady( function(){
	...
	haveRight('common/marketing.jsp');
	 querymarketingStore = new Ext.data.JsonStore({
...
iconCls : 'icon-select',
handler : function() {
querymarketingStore.proxy = new Ext.data.HttpProxy({
...
//	personelTypeStore.load();
	...
	var querymarketingGrid = new Ext.grid.GridPanel({
		store : querymarketingStore,
		sm : sm,
		columns : [ sm, rowNum,{
header : '产品编号',
dataIndex : 'idNumber',
...
width : 120 
		...
		tbar : [{
			...
			iconCls:'icon-del',
			handler : function() {
				del (querymarketingGrid, querymarketingStore, "t_product_info_delete.action", "idNumber", "删除信息");
			}
		}],
		bbar : [{
			xtype : 'paging',
	...
	showQueryPanel = new Ext.Panel({
		...
		items : [selectUnitForm, querymarketingGrid]
	});
	...
	querymarketingGrid.setWidth(showQueryPanel.getWidth());
	querymarketingGrid.setHeight(showQueryPanel.getHeight()-selectUnitForm.getHeight());
	
	

	
	addmarketingForm = new Ext.FormPanel({
		title :  '添加信息',
		frame : true,
		waitMsgTarget : true,
		labelAlign : 'right',
...
anchor : '90%'
		...
		buttons : [{
			...
			disabled : false,
			handler : function() {
				check(addmarketingForm, "t_product_info_addNewInfo.action", querymarketingStore, "添加信息");
				addmarketingWin.hide();	
			}
		}, {
			text : '取消',
			iconCls:'li-cancel',
			handler : function() {
				addmarketingWin.hide();
				addmarketingForm.form.reset();
			}
		}]

	});

	addmarketingWin = new Ext.Window({
		layout : 'form',
		width : 665,
		height : 800,
		plain : true,
		closable : false,
		items : addmarketingForm
	});

	addmarketing = function() {
		addmarketingWin.show();
		
//		Ext.getCmp("idNumber").focus(false, 1000);
	
	};
	
	updatemarketingReader = new Ext.data.JsonReader({
		root : 'tempList',
...
mapping : 'back'
	...
	updatemarketingForm = new Ext.FormPanel({
		title : '请输入您要修改的信息',
		frame : true,
		waitMsgTarget : true,
		reader : updatemarketingReader,
		items : [{
layout : 'column',
items : [
{
...
anchor : '90%'
		...
		buttons : [{
			...
			disabled : false,
			handler : function() {
				check(updatemarketingForm, "t_product_info_update.action", querymarketingStore, "修改信息");
				updatemarketingWin.hide();
			}
		}, {
			text : '取消',
			iconCls:'li-cancel',
			handler : function() {
				updatemarketingWin.hide();
				updatemarketingForm.form.reset();
			}
		}]
	});

	updatemarketingWin = new Ext.Window({
		layout : 'form',
		width : 665,
		height : 800,
		plain : true,
		closable : false,
		items : updatemarketingForm
	});

	update = function() {
		_record = querymarketingGrid.getSelectionModel().getSelected();
		flag = querymarketingGrid.getSelectionModel().getSelections();
		if (flag.length == 1) {
			updatemarketingWin.show();
					
			updatemarketingForm.getForm().load({
