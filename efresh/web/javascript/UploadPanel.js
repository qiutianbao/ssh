	...
	constructor : function(config) {
		...
		this.fileList = new Ext.grid.EditorGridPanel({
			...
			ds : new Ext.data.SimpleStore({
				fields : ['fileNo','fileId', 'fileName', 'fileSize', 'fileType', 'fileState']
			}),
	...
	startUpload : function() {
		...
		if (this.swfupload) {
			...
			var post_params = this.swfupload.settings.post_params;
			 ...
			 var test="";
			 this.fileList.getStore().each(
				function(record) {
					var fileName=record.data.fileName;
					test=test+encodeURI(record.data.fileName +":"+record.data.fileNo+",");
//					test=test+record.data.fileId+":"+record.data.fileNo+",";
//					test=test+record.data.fileNo+",";
				}
			)
			post_params.fileNo=test;
