var _ESONCalendar = window.ESONCalendar = {
	...
	onselect : function(d) {
		this.hotInput
				&& (this.hotInput.value = d.y + this.splitChar + d.m
						+ this.splitChar + d.d), this.hide()
	},
	addStyle : function() {
		var cssText = "#ESONCalendar_Win{background:" + this.color.caption
				+ ";position:absolute;z-index:9999}";
		cssText += "#ESONCalendar_caption{padding:3px;background:"
				+ this.color.caption + ";overflow:hidden;}";
		cssText += ".clear{clear:both}";
		cssText += "#selMonth{margin-right:5px;width:80px}";
		cssText += "#selYear{margin-right:3px;}";
		cssText += "#ESONCalendar_table{background:" + this.color.tablebg
				+ ";border-collapse:collapse;border:1px solid "
				+ this.color.border + "}";
		cssText += "#ESONCalendar_table th{border:1px " + this.color.border
				+ " solid}";
		...
		cssText += "#dateBox .unselected{cursor:pointer;background:"
				+ this.color.tablebg + ";}";
		cssText += "#dateBox .sunday{cursor:pointer;background:"
				+ this.color.tablebg + ";color:red}";
		cssText += "#dateBox .current,#dateBox .selected{cursor:pointer;background:"
				+ this.color.selectedbg
				+ ";color:"
				+ this.color.selectedco
				+ "}";
		cssText += "#ESONCalendar_foot{padding:2px 0 2px 0;line-height:130%;text-align:center;font-size:11px;color:"
				+ this.color.foot_co + ";background:" + this.color.border + "}";
		cssText += "#ESONCalendar_Win iframe{position:absolute;z-index:-1;top:0;left:0}";
		var STYLE = document.createElement('style');
		STYLE.setAttribute("type", "text/css");
		STYLE.styleSheet && (STYLE.styleSheet.cssText = cssText)
	...
	addUI : function() {
		...
		var _caption = this.uanv_tool_CE("DIV", "ESONCalendar_caption",
				this.Win);
		...
		selMonth.onchange = selYear.onchange = function() {
			ESONCalendar.dateUp(new Date(selYear.value, selMonth.value,
					ESONCalendar.d))
		};
		for ( var i = 0; i < 12; i++) {
		...
		for ( var i = 0; i < 7; i++) {
			var th = this.uanv_tool_CE("TH", false, tr, false, new String(
					this.weeks).charAt(i));
		}
		...
		;
		if (this.hasFoot) {
			this.foot = this.uanv_tool_CE("DIV", "ESONCalendar_foot", this.Win,
					false, this.footText);
			this.foot.innerHTML = "";
		}
		...
		document.onclick = document.body.onclick = function(e) {
			...
			if (src == ESONCalendar.hotInput
					|| ESONCalendar.uanv_tool_isIn(src, ESONCalendar.Win)) {
				return;
			}
			ESONCalendar.hide();
		};
	...
	dateUp : function(date, first) {
		...
		var m2d = 31, index = 1;
		this.y = date.getFullYear(), this.m = date.getMonth() + 1,
				this.d = date.getDate();
		...
		var isRN = (this.y % 4 == 0 && this.y % 4 != 100 || this.y % 100 == 0
				&& this.y % 400 == 0);
		...
		for ( var i = 0; i < 42; i++) {
			...
			_this.onmouseout = function() {
				if (this.className != "selected")
					this.className = this.isSunday ? "sunday" : "unselected"
			};
